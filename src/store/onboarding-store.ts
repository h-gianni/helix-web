...
const onboardingApi = {
  completeOnboarding: async (input: CompleteOnboardingInput): Promise<CompleteOnboardingResponse> => {
    const { data } = await apiClient.post<ApiResponse<CompleteOnboardingResponse>>('/onboarding', input);
...
export const useOnboardingStore = create<OnboardingState>((set, get) => ({
  ...
  setCurrentStep: (step) => {
    ...
    if (step === 2) { // Assuming team step is index 2
  ...
  syncTeamCategories: () => {
    ...
    const selectedCategoryIds = Object.keys(config.activities.selectedByCategory || {});
    ...
    if (selectedCategoryIds.length > 0 && config.teams.length > 0) {
      ...
      if (JSON.stringify(updatedTeams) !== JSON.stringify(config.teams)) {
        console.log('Updating teams with categories in sync operation', updatedTeams);
    ...
    mutationFn: async () => {
      ...
      const payload: CompleteOnboardingInput = {
        ...
        teams: config.teams.map(team => ({
          id: team.id || `temp-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`,
          name: team.name || '',
      ...
      console.log('Sending onboarding completion payload:', JSON.stringify(payload, null, 2));
...
export const useStepValidation = () => {
  ...
  const validateStep = (stepId: string): boolean => {
    switch (stepId) {
      ...
      case "activities":
        const hasSelectedActivities = (config.activities.selected || []).length > 0;
        const hasSelectedByCategory = Object.values(config.activities.selectedByCategory || {})
          .some(categoryActivities => categoryActivities.length > 0);
        
        return hasSelectedActivities || hasSelectedByCategory;
      ...
      case "team":
        ...
        console.log('Validating team step:', {
          ...
          selectedCategories: Object.keys(config.activities.selectedByCategory || {}).length
        });
