...
export function useOnboardingConfig() {
  ...
  const isGlobalActionsComplete = useCallback(() => {
    ...
    return Object.entries(selectedByCategory).some(([_, actions]) => actions.length >= 3);
  ...
  const isFunctionActionsComplete = useCallback(() => {
    ...
    return Object.entries(selectedByCategory).some(([_, actions]) => actions.length > 0);
  }, [config.activities.selectedByCategory]);
  
  const isMembersComplete = useCallback(() => {
    // Check localStorage for members
    try {
      const savedMembers = localStorage.getItem('onboarding_members');
      return savedMembers ? JSON.parse(savedMembers).length > 0 : false;
    } catch (error) {
  ...
  const isTeamsComplete = useCallback(() => {
    return config.teams && config.teams.length > 0 && 
      config.teams.every(team => 
        team.name.trim() !== '' && 
