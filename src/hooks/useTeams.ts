...
export function useTeams() {
  ...
  const getActionCountForCategory = useCallback(
  ...
  const setTeamFunctions = useCallback(
    (functions: string[]) => {
  ...
  const handleFunctionToggle = useCallback(
    (categoryId: string, checked: boolean) => {
      if (checked) {
        ...
        setTeamFunctions(
          currentTeam.functions.filter((id) => id !== categoryId)
        );
      }
    },
  ...
  const createTeam = useCallback(() => {
    ...
    const newTeam: Team = {
      ...
      functions: currentTeam.functions.map(
        (id) => getCategoryNameById(id) || ""
      ),
  ...
  return {
    ...
    allMembersAssigned:
      assignedMemberIds.length === members.length && members.length > 0,
    setTeamName,
