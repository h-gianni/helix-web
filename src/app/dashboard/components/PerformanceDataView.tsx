...
export default function PerformanceDataView({
  performers,
  teams,
}: PerformanceDataViewProps) {
  const { performanceCategories, getPerformanceCategory } =
    usePerformersStore();
  ...
  const performersByCategory = performanceCategories
    .map((category) => {
  ...
  const teamPerformance = teams
    .map((team) => {
      const teamMembers = performers.filter(
        (p) => p.teamId === team.id && p.ratingsCount > 0
      );
      const avgRating =
        teamMembers.length > 0
          ? teamMembers.reduce((sum, p) => sum + p.averageRating, 0) /
            teamMembers.length
          : 0;
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            ...
            <div className="p-4 border rounded-md text-center">
              <div className="text-2xl font-bold">
                {ratedPerformers.length > 0
                  ? Math.max(
                      ...ratedPerformers.map((p) => p.averageRating)
                    ).toFixed(1)
                  : "N/A"}
              </div>
